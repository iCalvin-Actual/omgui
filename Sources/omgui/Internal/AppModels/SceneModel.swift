...
class SceneModel {
    ...
    func fetchBio(_ address: AddressName) throws {
        Task { [weak self] in
    ...
    func fetchStatuses(_ addresses: [AddressName]) throws {
        Task { [weak self] in
            guard let self else { return }
    ...
    func fetchProfile(_ address: AddressName) throws {
        Task { [weak self] in
            guard let self else { return }
    ...
    func fetchProfileContent(_ address: AddressName) throws {
        Task { [weak self] in
            guard let self else { return }
    ...
    func fetchNow(_ address: AddressName) throws {
        Task { [weak self] in
            guard let self else { return }
            try fetchNowGarden()
    ...
    func fetchPURLS(_ address: AddressName) throws {
        try fetchPURLS([address])
    }
    
    func fetchPURLS(_ addresses: [AddressName]) throws {
        Task { [weak self] in
            guard let self else { return }
    ...
    func fetchPURL(_ address: AddressName, title: String) throws {
        Task { [weak self] in
            guard let self else { return }
            let credential = accountModel.credential(for: address)
    ...
    func fetchPastes(_ address: AddressName) throws {
        try fetchPastes([address])
    }
    
    func fetchPastes(_ addresses: [AddressName]) throws {
        Task { [weak self] in
            guard let self else { return }
            for address in addresses {
                let credential = accountModel.credential(for: address)
    ...
    func fetchInfo(_ address: AddressName) {
        Task { [weak self] in
            guard let self else { return }
            fetchIcon(address)
...
//        }
    ...
    func saveBlocked(_ addresses: [AddressName], for address: AddressName) {
        Task { [weak self] in
            guard let self else { return }
    ...
    func saveFollowing(_ addresses: [AddressName], for address: AddressName) {
        Task { [weak self] in
            guard let self else { return }
            guard let credential = accountModel.credential(for: address) else {
                return
